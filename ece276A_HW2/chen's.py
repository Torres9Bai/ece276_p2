# -*- coding: utf-8 -*-
"""
Created on Thu Feb 21 05:25:18 2019
...
def get_angular_velocity(angular_velocity_lasttime, time):#就用最新的IMU-Time即可。
    ...
    imu_tmp = yaw_data[2,np.where(imu_tmp_stamps >= encoder_stamps[time])]
    #if 
    omega =  (np.sum(yaw_data)/len(imu_tmp[0,:]))
    return omega, angular_velocity_lasttime



def get_theta(angular_velocity, particles, time_interval):
    return particles[:,2] + angular_velocity * time_interval
...
def get_updated_position(velocity, angular_velocity,time_interval,particles,Number):
    ...
    particles[:,0] = particles[:,0] + velocity * time_interval*math.sin(angular_velocity*time_interval/2)/(angular_velocity*time_interval\
    /2)*np.cos(particles[:,2] + angular_velocity*time_interval/2)

    particles[:, 1] = particles[:,1] + velocity * time_interval * math.sin(angular_velocity * time_interval /2)/(angular_velocity * \
...
def predict(time, particles, time_interval,Number):
    ...
    particles[:, 2] += get_theta(average_angular_velocity,particles, time_interval)
    average_velocity = get_velocity(time)
    particles = get_updated_position(average_velocity,average_angular_velocity,time_interval,particles, Number)
...
if __name__ == '__main__':
    ...
    y_robot_world = 0
    Number = 6
    particles = set_particles((-0.4, 0.4), (-0.4, 0.4), (0, 0), 6) # setup Particles
    ...
    yaw_data = signal.filtfilt(b, a, imu_angular_velocity)
    ...
    start_time = tic()
    for encoder_time in range(860,869): # 读取所有的Lidar数据
        ...
        particles = predict(encoder_time, particles, time_interval, Number)  # (X，Y) Unit:meters
