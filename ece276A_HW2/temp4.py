# -*- coding: utf-8 -*-
"""
Created on Fri Feb 22 16:21:15 2019
...
N=1
particles=create_particles([-0.01,0.01], [-0.01,0.01],0,N)
...
for i in range (1):
    ...
    Y=np.zeros([2,1])
    YF=np.zeros([2,1])
    for j in range (np.size(ranges)):
        Yi=L2W(ranges[j]/0.05, angles[j], particles[0,0]/0.05, particles[0,1]/0.05, particles[0,2])
        xi=Yi[0]
        yi=Yi[1]
        Y=np.hstack((Y,np.array([[xi],[yi]])))
        YF=np.hstack((YF,bresenham2D(particles[0,0]/0.05, particles[0,1]/0.05, xi, yi)))
    Y=Y[:,1:]
    YF=YF[:,1:]
    xs0=Y[0,:]
    ys0=Y[1,:]
